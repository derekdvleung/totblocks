...
// Single Octahedron
//oBlockOne(apex = MINUS, pegs = false);
...
//TOT();
//sepioliteChild()
//assembledTripleChain();
//printTOTPx();
/*translate([0,-23,0])
oModule2();
translate([0,23,0])
oModule2();*/
//chloriteBrucitePeg(bruciteHeight);
//sepioliteChild()
 //assembledTripleChain();
 //simpTOT();
//protopyribole(3,3,3,1);
//TOTChild(3, PLUS){
////tPlusModule2();
//	//render()
//	tBlock(3, MINUS);
...
/*n = 2;
m = 3*n-1;
//protoTO (n, rows = 2, columns = 2){
	TOTChild(n,apex = PLUS,m){
		tBlock(n, apex =  MINUS);
		oBlock(m ,apex = PLUS);
		tBlock(n, apex = PLUS);
	}*/
	
	//TOTChild(n,apex = MINUS,m = 2){
	//	tBlock(n, apex = MINUS);
	//	oBlock(m,apex = MINUS);
	//	tBlock(n, apex =  PLUS);
	//}
//}
	//chevronSlotY(pegYLength, pegRadius,toleranceXY);
//rotate([90,0,0])
//	pegDoubleCham(micaHeight/2+pegZLength, pegRadius,micaHeight/2+pegZLength);


	//oBlockDual(m=2,apex = PLUS, dual = 1);
	//translate([0,0,10])
//	translate([0,0,tOffset])
	//tBlockDual2(n, apex= MINUS, dual = 1);
		//translate([0,0,1])
	//tBlockDual2(n=1, apex= PLUS, dual = 1);
	//}
	//translate([1,0,0])
